      ...
      <form id="document-edit-form" action="{{ route('admin.document-type.update', $documentType->id) }}" method="post">
          ...
          <div class="form-group d-none">
            ...
            <div class="form-check-inline">
              ...
              <input type="radio" class="form-check-input" name="document_required[]" value="1" @if ($documentType->is_required == 1) checked  @endif >Yes
              </label>
            </div>
            <div class="form-check-inline">
              <label class="form-check-label">
                <input type="radio" class="form-check-input" name="document_required[]" value="0" @if ($documentType->is_required == 0) checked  @endif >No
              </label>
            </div>
          </div>
        ...
        @else
          <div class="form-group delete " >
              <form  id="delete-form" method="POST" action="{{ route('admin.document-type.destroy', $documentType->id) }}" >
                  @csrf
                  @method('DELETE')
