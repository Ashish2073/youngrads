<form method="POST" action="{{ route('education-update', $academics->id) }}" id="education-history-edit">
    ...
    $studyLevel = DB::table('study_levels')->where('id',$academics->study_levels_id)->select('id','name')->orderBy('sequence', 'desc')->get();
    $country = DB::table('countries')->where('id', $academics->country)->select('name', 'id')->orderBy('name', 'asc')->get();
    @endphp
    <div class="row">
        <div class="col-md-6">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('board') {{ errCls() }} @enderror" id="board" name="board"
                    value="{{ old('board', $academics->board_name) }}">
                @error('board')
                ...
                <div class="col-8">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('marks') {{ errCls() }} @enderror" id="marks"
                            name="marks" value="{{ old('marks', $academics->marks) }}">
                        @error('marks')
                        <p class="text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select data-style="border-light bg-white" class="form-control select" id="marks-unit"
                            ...
                            @foreach (config('setting.units') as $value => $text)
                                <option value="{{ $value }}" @if ($academics->marks_unit == $value) selected
                            @endif>{{ $text }}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('start_date') {{ errCls() }} @enderror" id="start-date"
                    name="start_date" value="{{ old('start_date', date('d-F-Y', strtotime($academics->start_date))) }}">
                @error('start_date')
                <p class="text-danger">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="col-md-6">
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('language') {{ errCls() }} @enderror" id="language"
                    name="language" value="{{ old('language', $academics->language) }}">
                @error('language')
                <p class="text-danger">{{ $message }}</p>
                @enderror
            </div>
            <div class="form-group">
                ...
                <select data-style="border-light bg-white" name="country" class="select form-control"
                    ...
                    @foreach ($countries as $country)
                        <option value="{{ $country->id }}" @if (old('address_country') == $country->id || $studyCountry == $country->id)
                            selected
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('end_date') {{ errCls() }} @enderror" id="end-date"
                    name="end_date" value="{{ old('end_date', date('d-F-Y', strtotime($academics->end_date))) }}">
                @error('end_date')
                <p class="text-danger">{{ $message }}</p>
                @enderror
            </div>
    ...
    <div class="row">
        <div class="col-12">
            <button type="submit" class="btn btn-primary" id="edu-edit-btn">Update</button>
        </div>
    </div>
</form>
