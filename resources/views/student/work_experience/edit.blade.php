<form action="{{ route('experence-update', $workExperience->id) }}" class="my-2" method="POST" id="work-experience-edit">
    @csrf
    @method('PUT')
    <div class="row">
        <div class="col-md-6 col-12">
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('organization') {{ errCls() }} @enderror"
                    ...
                    value="{{ old('organization', $workExperience->organization) }}"
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('job_profile') {{ errCls() }} @enderror"
                    id="job_profile" name="job_profile" value="{{ old('job_profile', $workExperience->position) }}"
            ...
            <div class="form-group">
                ...
                <input type="text"
                    ...
                    id="working_from" name="working_from"
                    value="{{ old('working_from', date('d-F-Y', strtotime($workExperience->work_from))) }}"
        ...
        <div class="col-md-6 col-12">
            <div class="form-group">
                ...
                <input type="text" class="form-control @error('position') {{ errCls() }} @enderror"
                    id="position" name="position" value="{{ old('position', $workExperience->position) }}"
            ...
            <div class="form-group">
                ...
                <select class="form-control @error('mode') {{ errCls() }} @enderror" id="mode"
                    ...
                    @foreach (config('setting.sallaryMode') as $sallaryMode)
                        <option value="{{ $sallaryMode }}" @if ($workExperience->sallery_mode == $sallaryMode) selected @endif>
                            {{ $sallaryMode }}</option>
                    @endforeach
                </select>
                @error('first_name')
            ...
            <div class="form-group">
                @php
                    if (isset($workExperience->work_upto)) {
                        $date = date('d-m-Y', strtotime($workExperience->work_upto));
                    } else {
                        $date = '';
                    }
                @endphp
                <label for="working_upto">Working upto<span class="required text-danger">*</span></label>
                <input {{ old('current_working', $workExperience->is_working) == 1 ? 'disabled' : 'required' }}
                    type="text"
                    class="form-control work-date @error('working_upto') {{ errCls() }} @enderror work-upto"
                    id="working_upto" name="working_upto" value="{{ old('working_upto', $date) }}"
    ...
    <div class="vs-checkbox-con vs-checkbox-primary my-1">
        <input type="checkbox" value="1" name="current_working" @if ($workExperience->is_working == 1) Checked @endif
            class="current_working">
