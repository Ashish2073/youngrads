...
<ul class="menu-content">
    @if(isset($menu))
        @foreach($menu as $submenu)
            ...
            @endphp
            @if( !isset($submenu['permission']) || empty($submenu['permission']) || (isset($submenu['permission']) && auth()->user()->can($submenu['permission'])) )
                <li
                        class=" {{ request()->route()->getName() == $submenu['route'] ? "active" : "" }} ">
                    <a href="{{ isset($submenu['route']) ? route($submenu['route']) : "" }}">
                        ...
                        <span class="menu-title"
                              data-i18n="{{ $submenuTranslation }}">{{ __($submenu['menu_name']) }}</span>
                    </a>
